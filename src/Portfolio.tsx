  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.from("[data-hero-eyebrow]", { opacity: 0, y: 16, duration: 0.6 })
        .from("[data-hero-title] > span", { opacity: 0, y: 30, duration: 0.85, stagger: 0.14 }, "-=0.3")
      ...
      const revealSafety = window.setTimeout(() => {
        reveals.forEach((node) => {
          ...
          if (parseFloat(getComputedStyle(node).opacity) < 0.05 && r.top < window.innerHeight * 1.2) {
  ...
  return (
    <div className="portfolio" ref={root}>
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,420px) 1fr", gap: "clamp(24px,5vw,60px)", alignItems: "center", minHeight: "min(80vh,740px)" }}>
          ...
          <div data-hero-frame="" style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420 }}>
            ...
            <PixelPortrait src={PROFILE.photo} />
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            ...
            <h1 data-hero-title="" style={{ margin: 0, lineHeight: 0.82 }}>
              <span style={{ display: "block", position: "relative", marginLeft: "clamp(-210px,-16vw,-70px)" }}>
                <span aria-hidden="true" style={{ position: "absolute", left: 0, top: 0, fontFamily: "var(--pixel)", fontWeight: 700, fontSize: "clamp(4rem,12.5vw,9.5rem)", color: "rgba(31,70,255,0.22)", letterSpacing: "0.01em", transform: "translate(11px,11px)", zIndex: 0 }}>
                  {PROFILE.name}
                </span>
              ...
              <span style={{ display: "block", fontFamily: "var(--sans)", fontWeight: 500, fontSize: "clamp(1.2rem,2.3vw,1.85rem)", lineHeight: 1.2, marginTop: "0.55em", maxWidth: "22ch" }}>
                {PROFILE.statement}
              </span>
            </h1>
            ...
            <div data-hero-meta="" style={{ display: "flex", flexWrap: "wrap", alignItems: "stretch", gap: 12, marginTop: 32 }}>
              ...
              <a href="#" className="btn-ghost" style={{ ...btnGhost, textTransform: "uppercase", fontSize: 12, letterSpacing: "0.08em", padding: "14px 22px" }}>
                Download CV
              </a>
      ...
      <section id="about" data-screen-label="About" style={{ position: "relative", overflow: "hidden" }}>
        <div style={{ ...container, padding: "clamp(80px,13vh,150px) clamp(20px,5vw,64px)" }}>
          <span style={{ ...watermark, top: "clamp(-30px,-3vw,0px)", right: "clamp(-30px,-2vw,0px)", color: "rgba(31,70,255,0.07)" }}>01</span>
          ...
          <p data-reveal="" style={{ position: "relative", zIndex: 1, margin: "34px 0 0", marginLeft: "auto", maxWidth: "50ch", fontSize: 16, lineHeight: 1.7, color: "var(--muted)", textAlign: "right" }}>
      ...
      <section id="work" data-screen-label="Work" style={{ position: "relative", overflow: "hidden", background: "var(--bg-2)", borderTop: "1px solid var(--ink)" }}>
        <div style={container}>
          <span style={{ ...watermark, top: "clamp(20px,4vw,60px)", left: "clamp(-20px,-1vw,0px)", color: "rgba(11,11,12,0.05)" }}>02</span>
      ...
      <section id="skills" data-screen-label="Skills" style={{ position: "relative", overflow: "hidden", borderTop: "1px solid var(--ink)" }}>
        <div style={container}>
          <span style={{ ...watermark, top: "clamp(-20px,-1vw,10px)", right: "clamp(-20px,-1vw,0px)", color: "rgba(31,70,255,0.07)" }}>03</span>
          ...
          <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "clamp(24px,4vw,48px)", alignItems: "start" }}>
            {SKILLS.map((group, i) => (
              <div key={group.label} data-reveal="" style={{ marginTop: SKILL_OFFSETS[i] ?? 0 }}>
      ...
      <section id="experience" data-screen-label="Experience" style={{ position: "relative", overflow: "hidden", background: "var(--bg-2)", borderTop: "1px solid var(--ink)" }}>
        <div style={container}>
          <span style={{ ...watermark, bottom: "clamp(-30px,-3vw,-10px)", left: "clamp(-20px,-1vw,0px)", color: "rgba(11,11,12,0.05)" }}>04</span>
      ...
      <section id="contact" data-screen-label="Contact" style={{ position: "relative", overflow: "hidden", borderTop: "1px solid var(--ink)" }}>
        <div style={{ ...container, paddingBottom: "clamp(48px,8vh,90px)" }}>
          ...
          <h2 data-reveal="" style={{ position: "relative", margin: 0, lineHeight: 0.84 }}>
            <span aria-hidden="true" style={{ position: "absolute", left: 0, top: 0, fontFamily: "var(--pixel)", fontWeight: 700, fontSize: "clamp(3.2rem,12vw,9rem)", color: "rgba(31,70,255,0.22)", transform: "translate(10px,10px)", letterSpacing: "0.01em" }}>
              LET&apos;S BUILD.
            </span>
          ...
          <div data-reveal="" style={{ position: "relative", zIndex: 2, display: "flex", flexWrap: "wrap", gap: 12, marginTop: "clamp(28px,4vh,44px)" }}>
            ...
            <a href={PROFILE.github} target="_blank" rel="noopener noreferrer" className="btn-ghost" style={btnGhost}>
              github / {PROFILE.githubHandle}
            </a>
            <a href="#" className="btn-ghost" style={btnGhost}>download cv</a>
          </div>

          {/* contribution band */}
          <div data-reveal="" style={{ position: "relative", zIndex: 2, marginTop: "clamp(48px,8vh,90px)", border: "1px solid var(--ink)", background: "var(--bg)", padding: "clamp(22px,4vw,38px)", boxShadow: "10px 10px 0 rgba(31,70,255,0.14)" }}>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
              ...
              <a href={PROFILE.github} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--accent)", textDecoration: "none" }}>
